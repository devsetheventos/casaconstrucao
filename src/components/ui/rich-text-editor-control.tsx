...
type EditorChain = {
  run: () => EditorChain
  [key: string]: (...args: unknown[]) => EditorChain
}
...
const getChain = (editor: Editor) => editor.chain().focus() as unknown as EditorChain
const getCan = (editor: Editor) => editor.can() as unknown as EditorCan

export interface BaseControlConfig<TProps extends object = Record<string, never>> {
  label: string
  icon?: React.ElementType
  isDisabled?: (editor: Editor) => boolean
  getProps?: (editor: Editor) => Partial<TProps>
}
...
export interface BooleanControlConfig extends BaseControlConfig<IconButtonProps> {
  icon: React.ElementType
...
export interface SelectControlConfig extends BaseControlConfig<Select.RootProps> {
...
export interface SwatchControlConfig extends BaseControlConfig<IconButtonProps> {
...
export const FontFamily = createSelectControl({
  ...
  command: (editor, value) =>
    value === "default"
      ? getChain(editor).unsetFontFamily().run()
      : getChain(editor).setFontFamily(value).run(),
})
...
export const Bold = createBooleanControl({
  label: "Bold",
  icon: LuBold,
  command: (editor) => getChain(editor).toggleBold().run(),
...
export const Italic = createBooleanControl({
  label: "Italic",
  icon: LuItalic,
  command: (editor) => getChain(editor).toggleItalic().run(),
...
export const Underline = createBooleanControl({
  label: "Underline",
  icon: LuUnderline,
  command: (editor) => getChain(editor).toggleUnderline().run(),
...
export const Strikethrough = createBooleanControl({
  ...
  command: (editor) => getChain(editor).toggleStrike().run(),
...
export const Code = createBooleanControl({
  label: "Code",
  icon: LuCode,
  command: (editor) => getChain(editor).toggleCode().run(),
...
export const Subscript = createBooleanControl({
  label: "Subscript",
  icon: LuSubscript,
  command: (editor) => getChain(editor).toggleSubscript().run(),
...
export const Superscript = createBooleanControl({
  label: "Superscript",
  icon: LuSuperscript,
  command: (editor) => getChain(editor).toggleSuperscript().run(),
...
export const H1 = createBooleanControl({
  label: "H1",
  icon: LuHeading1,
  command: (editor) => getChain(editor).toggleHeading({ level: 1 }).run(),
  getVariant: (editor) =>
...
export const H2 = createBooleanControl({
  label: "H2",
  icon: LuHeading2,
  command: (editor) => getChain(editor).toggleHeading({ level: 2 }).run(),
  getVariant: (editor) =>
...
export const H3 = createBooleanControl({
  label: "H3",
  icon: LuHeading3,
  command: (editor) => getChain(editor).toggleHeading({ level: 3 }).run(),
  getVariant: (editor) =>
...
export const H4 = createBooleanControl({
  label: "H4",
  icon: LuHeading4,
  command: (editor) => getChain(editor).toggleHeading({ level: 4 }).run(),
  getVariant: (editor) =>
...
export const BulletList = createBooleanControl({
  label: "Bullet List",
  icon: LuList,
  command: (editor) => getChain(editor).toggleBulletList().run(),
...
export const OrderedList = createBooleanControl({
  label: "Ordered List",
  icon: LuListOrdered,
  command: (editor) => getChain(editor).toggleOrderedList().run(),
...
export const Blockquote = createBooleanControl({
  label: "Blockquote",
  icon: LuQuote,
  command: (editor) => getChain(editor).toggleBlockquote().run(),
...
export const Hr = createBooleanControl({
  ...
  icon: LuMinus,
  command: (editor) => getChain(editor).setHorizontalRule().run(),
  getVariant: (editor) => (editor.isActive("blockquote") ? "subtle" : "ghost"),
})
...
export const Link = createBooleanControl({
  ...
  command: (editor) => {
    ...
    if (url)
      getChain(editor).extendMarkRange("link").setLink({ href: url }).run()
  },
...
export const Unlink = createBooleanControl({
  label: "Unlink",
  icon: LuLink2,
  command: (editor) => getChain(editor).unsetLink().run(),
  getVariant: (editor) => (editor.isActive("link") ? "subtle" : "ghost"),
})
...
export const AlignLeft = createBooleanControl({
  label: "Align Left",
  icon: LuAlignLeft,
  command: (editor) => getChain(editor).setTextAlign("left").run(),
  getVariant: (editor) =>
...
export const AlignCenter = createBooleanControl({
  label: "Align Center",
  icon: LuAlignCenter,
  command: (editor) => getChain(editor).setTextAlign("center").run(),
  getVariant: (editor) =>
...
export const AlignJustify = createBooleanControl({
  label: "Align Justify",
  icon: LuAlignJustify,
  command: (editor) => getChain(editor).setTextAlign("justify").run(),
  getVariant: (editor) =>
...
export const AlignRight = createBooleanControl({
  label: "Align Right",
  icon: LuAlignRight,
  command: (editor) => getChain(editor).setTextAlign("right").run(),
  getVariant: (editor) =>
...
export const Undo = createBooleanControl({
  label: "Undo",
  icon: LuRotateCcw,
  command: (editor) => getChain(editor).undo().run(),
  isDisabled: (editor) => !getCan(editor).undo(),
  getVariant: (editor) => (editor.isActive("link") ? "subtle" : "ghost"),
})
...
export const Redo = createBooleanControl({
  label: "Redo",
  icon: LuRotateCw,
  command: (editor) => getChain(editor).redo().run(),
  isDisabled: (editor) => !getCan(editor).redo(),
  getVariant: (editor) => (editor.isActive("link") ? "subtle" : "ghost"),
})
...
export const Highlight = createSwatchControl({
  ...
  command: (editor, color) => getChain(editor).toggleHighlight({ color }).run(),
  icon: LuHighlighter,
  showRemove: true,
  onRemove: (editor) => getChain(editor).unsetHighlight().run(),
})
...
export const TextStyle = createSelectControl({
  ...
  command: (editor, value) => {
    if (value === "paragraph") {
      getChain(editor).setParagraph().run()
    } else if (value === "heading1") {
      getChain(editor).toggleHeading({ level: 1 }).run()
    } else if (value === "heading2") {
      getChain(editor).toggleHeading({ level: 2 }).run()
    } else if (value === "heading3") {
      getChain(editor).toggleHeading({ level: 3 }).run()
    } else if (value === "blockquote") {
      getChain(editor).toggleBlockquote().run()
    } else if (value === "horizontalRule") {
      getChain(editor).setHorizontalRule().run()
    }
  },
  renderValue: (value, option) => {
    ...
    const style = textStyle[value as keyof typeof textStyle] ?? {}
    return <Box {...style}>{option?.label || "Paragraph"}</Box>
  },
})
